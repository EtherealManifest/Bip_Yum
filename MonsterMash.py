...
class Monster(pygame.sprite.Sprite):
    ...
    def __init__(self):
        ...
        #logging.info("NEW ENEMY CREATED:\n" + self.statBlock.showStats())
    ...
    def takeDamage(self, player):
        if self.hitTick == HITTIME:
            #logging.info("Monster took " + str(player.statBlock.ATTACK) + " damage!")
            self.statBlock.HEALTH = self.statBlock.HEALTH - player.statBlock.ATTACK
            #logging.info("Monster Health is now " + str(self.statBlock.HEALTH) + " out of " +  str(self.statBlock.TOTALHEALTH))
    ...
    def update(self, slime):
        if self.statBlock.HEALTH > 0:
            ...
            #logging.info(slime.rectangle() + "\n\n" +  self.rectangle())
            #logging.info("\nSlime position(slime.position): " + str(slime.position) + 
            #            "\nSlime Position(slime.statBlock.pos): " + str(slime.statBlock.pos) +
            #            "\nMonster position(self.position): " + str(self.position) + 
            #            "\nMonster Position(self.statBlock.pos): " + str(self.statBlock.pos))
            #for this part, there is a wierd line about assigning moverate to movehit. this is a system that makes sure that he moves in the 
            #right direction when he is hit. 
            if self.isHit:
            ...
            #make sure that the monster doesnt go inside of slime. IF they arent Overlapping, move enemy
                ...
                #logging.info("not overlapping, should be moving")
                #for this part, there is a wierd line about assigning moverate to movehit. this is a system that makes sure that he moves in the 
                #right direction when he is hit.
                if self.monsterX > slime.slimex:
